...
class Account:
    ...
    def register(request):
        ...
        if usertype not in ['100', '1000', 100, 1000]:
            return JsonResponse({'ret': 1, 'msg': '你没有权限注册该类账号！'}, json_dumps_params={'ensure_ascii': False})
    ...
    def addAccount(request):
        ...
        if 'is_login' not in request.session:
            return JsonResponse({'ret': 302, 'msg': '未登录'}, status=302, json_dumps_params={'ensure_ascii': False})

        if request.session['usertype'] != 1:
            return JsonResponse({'ret': 1, 'msg': '请使用管理员账号进行该操作！'}, json_dumps_params={'ensure_ascii': False})

        usertype = request.params.get('usertype')

        if usertype not in ['1', '100', '1000', 1, 100, 1000]:
            return JsonResponse({'ret': 1, 'msg': '账号类型不存在！'}, json_dumps_params={'ensure_ascii': False})
    ...
    def modifyAccount(request):
        ...
        if request.session['usertype'] == 1:
            ...
            if request.params['user_id'] != request.session['user_id']:
                return JsonResponse({'ret': 1, 'msg': '不能修改别人的信息！'}, json_dumps_params={'ensure_ascii': False})

            else:
                if 'studentNo' in request.params or 'gradeNo' in request.params or 'classNo' in request.params or 'major' in request.params:
                    return JsonResponse({'ret': 1, 'msg': '请联系管理员修改！'}, json_dumps_params={'ensure_ascii': False})
